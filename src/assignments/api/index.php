...
function getAllAssignments(PDO $db): void
{
    ...
    // FROM assignments
    $sql='SELECT id, title, description, due_date, files, created_at,
    updated_at FROM assignments';
    $params=[];
    ...
    $sort=(!empty($_GET['sort']) && in_array ($_GET['sort'],$allowedSort,true))
        ? $_GET['sort']
        : 'due_date';
    ...
    $order=(!empty($_GET['order'])&& in_array(strtolower($_GET['order']),
    $allowedOrder, true))
...
function getAssignmentById(PDO $db, $id): void
{
    ...
    if (empty($id) || !is_numeric($id)){
        sendResponse(['success' => false,'message' => 'Inavlid or missing id.'],400);
    }
...
function createAssignment(PDO $db, array $data): void
{
    ...
    if (empty($data['title']) || !isset($data['description']) || empty($data['due_date'])) {
...
function updateAssignment(PDO $db, array $data): void
{
    ...
    if(empty($data['id']) || !is_numeric($data['id'])){
        sendResponse(['success'=>false,'message'=>'A numeric ID is required.'],400);
    }
    ...
    if (isset($data['files'])) {
        ...
        $params[':files'] = is_array($data['files']) ? json_encode($data['files']) : json_encode([]);
    }    
...
function getCommentsByAssignment(PDO $db, $assignmentId): void
{
    ...
    $stmt->execute([':assignment_id' => (int) $assignmentId]);
...
function createComment(PDO $db, array $data): void
{
    ...
    // Otherwise sendResponse HTTP 500.
        ...
        $newComment = $fetch->fetch(PDO::FETCH_ASSOC);
...
try {
    ...
    error_log('PDOException in api/index.php: '.$e->getMessage());
    ...
    error_log('Exception in api/index.php: '.$e->getMessage());
    sendResponse(['success'=>false,'message'=>'An unexcepted error occured.'],500);
}
